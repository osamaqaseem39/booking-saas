...
function normalizeTurfNamedCourtRows(raw: unknown): NamedCourt[] {
  const rows = Array.isArray(raw) ? raw : [];
  const out: NamedCourt[] = [];
  for (const item of rows) {
    ...
    if (!id || !name) continue;
    const supportedSports = Array.isArray(row.supportedSports)
      ? row.supportedSports.filter((s): s is string => typeof s === 'string')
      : [];
    const supportsCricket =
      row.supportsCricket === true || supportedSports.includes('cricket');
    out.push({
      id,
      name,
      tenantId: typeof row.tenantId === 'string' ? row.tenantId : undefined,
      businessLocationId:
        typeof row.businessLocationId === 'string'
...
export type TableTennisCourtMeta = {
  roomType?: 'indoor' | 'covered' | 'outdoor';
  playSurface?: string;
  tableModel?: string;
  playAreaLengthM?: number;
  playAreaWidthM?: number;
...
export type TableTennisCourtDetail = Partial<CreateTableTennisCourtBody> & {
  ...
  businessLocationId?: string | null;
  imageUrls?: string[];
  meta?: TableTennisCourtMeta | null;
  createdAt?: string;
  updatedAt?: string;
};
...
export type UpdateTableTennisCourtBody = Partial<
  Omit<CreateTableTennisCourtBody, 'businessLocationId' | 'meta'>
> & { meta?: TableTennisCourtMeta | null };
