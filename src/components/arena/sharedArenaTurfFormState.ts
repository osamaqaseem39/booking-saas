...
export function sharedDetailToFormState(
  ...
  return {
    ...
    slotDuration: strFromApi(d.slotDurationMinutes),
...
export function sharedTurfFormStateToPayload(
  ...
  const slotM = parseIntOpt(s.slotDuration);
  const buf = parseIntOpt(s.bufferMinutes);
  ...
  return {
    ...
    bufferBetweenSlotsMinutes: buf,
    allowParallelBooking: s.allowParallelBooking || undefined,
